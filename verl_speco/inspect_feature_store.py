...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inspect a torch_shard draft feature store.")
    parser.add_argument("path", help="Feature store directory containing manifest.jsonl and shard .pt files.")
    parser.add_argument("--max-samples", type=int, default=32, help="Maximum number of samples to inspect.")
    parser.add_argument("--show-ok", action="store_true", help="Print valid samples as well as invalid samples.")
    parser.add_argument("--strict-exit", action="store_true", help="Exit with code 1 when invalid samples are found.")
...
def _sample_summary(sample: dict[str, Any]) -> dict[str, str]:
    keys = ["input_ids", "loss_mask", "hidden_states", "last_hidden_states", "target_logprobs", "position_ids"]
...
def _sample_issues(sample: dict[str, Any]) -> list[str]:
    ...
    if input_ids is not None:
        if input_ids.dim() > 2 or (input_ids.dim() == 2 and 1 not in input_ids.shape):
            issues.append(f"input_ids should be 1D or singleton-2D, got {_shape(input_ids)}")
        seq_len = int(input_ids.numel())
    if loss_mask is not None:
        if loss_mask.numel() != seq_len:
            issues.append(f"loss_mask length {loss_mask.numel()} does not match input_ids length {seq_len}")
    if torch.is_tensor(hidden_states):
        if hidden_states.dim() == 3 and hidden_states.size(0) == 1:
            ...
            hidden_len = -1
            issues.append(f"hidden_states should be [seq, hidden] or [1, seq, hidden], got {_shape(hidden_states)}")
        if seq_len is not None and hidden_len >= 0 and hidden_len < max(seq_len - 1, 1):
            issues.append(f"hidden_states length {hidden_len} is too short for input_ids length {seq_len}")
    elif isinstance(hidden_states, (list, tuple)):
        for idx, tensor in enumerate(hidden_states):
            if not torch.is_tensor(tensor):
                issues.append(f"hidden_states[{idx}] is not a tensor: {type(tensor).__name__}")
            elif tensor.dim() not in {2, 3}:
                issues.append(f"hidden_states[{idx}] has unexpected shape {_shape(tensor)}")
    else:
        issues.append(f"hidden_states is not a tensor/list: {type(hidden_states).__name__}")

    if position_ids is not None:
        if not torch.is_tensor(position_ids):
            issues.append(f"position_ids is not a tensor: {type(position_ids).__name__}")
