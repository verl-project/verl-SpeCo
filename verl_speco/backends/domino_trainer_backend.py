...
class DominoTrainingModel(DFlashTrainingModel):
    ...
    def _sample_anchor_positions(self, seq_len: int, loss_mask: torch.Tensor, device: torch.device):
        ...
        indices = self._cached_arange("domino_anchor_indices", num_candidates, device).unsqueeze(0).expand(bsz, -1)
    ...
    def _build_label_tensors(self, *, input_ids, loss_mask, anchor_positions, block_keep_mask):
        bsz, seq_len = input_ids.shape
        device = input_ids.device
        n_blocks = anchor_positions.shape[1]
        label_offsets = self._cached_arange("domino_label_offsets", self.block_size, device, view_shape=(1, 1, -1)) + 1
    ...
    def forward(self, input_ids, hidden_states_list, loss_mask, lm_head_weight):
        ...
        if self.loss_decay_gamma is not None and self.loss_decay_gamma > 0:
            positions = self._cached_arange("domino_decay_positions", self.block_size, device, view_shape=(1, 1, -1))
        ...
        gru_out, _ = self.draft_model.prefix_gru(block_emb.reshape(bsz * n_blocks, self.block_size, -1))
        gru_out = gru_out.reshape(bsz, n_blocks, self.block_size, -1)

        pos_in_block = (
            self._cached_arange("domino_pos_in_block", self.block_size, device, view_shape=(1, 1, -1))
        ...
        with torch.no_grad():
            ...
            base_accuracy = base_correct.float().sum() / binary_eval_mask.float().sum().clamp(min=1.0)
        ...
        diagnostics = {
            ...
            "sampled_vocab_size": torch.tensor(float(lm_head_weight.shape[0]), dtype=torch.float32, device=device),
            ...
            "local_ploss_sum": (loss_per_token * binary_eval_mask.float()).sum().detach(),
            # Domino-specific diagnostics.
            "domino_final_loss": final_loss.detach() if torch.is_tensor(final_loss) else torch.tensor(0.0, device=device),
            "domino_base_loss": base_loss.detach() if torch.is_tensor(base_loss) else torch.tensor(0.0, device=device),
            "domino_base_accuracy": base_accuracy.detach(),
            "domino_lambda_base": torch.tensor(float(lambda_base), dtype=torch.float32, device=device),
        }
...
class DominoTrainerBackend(DFlashTrainerBackend):
    ...
    def _training_value(self, training_cfg, domino_key: str, dflash_key: str, default: Any):
    ...
    def _build_fallback_config(self, target_hf_config):
        ...
        hidden_size_cfg = self._training_value(training_cfg, "domino_hidden_size", "dflash_hidden_size", None)
        hidden_size = int(hidden_size_cfg if hidden_size_cfg is not None else target_text_config.hidden_size)
        num_context_layers = int(self._training_value(training_cfg, "domino_num_target_layers", "dflash_num_target_layers", 5))
        target_num_hidden_layers = int(getattr(target_text_config, "num_hidden_layers", 36))
        mask_token_id_cfg = self._training_value(training_cfg, "domino_mask_token_id", "dflash_mask_token_id", None)
        mask_token_id = int(mask_token_id_cfg if mask_token_id_cfg is not None else target_text_config.vocab_size - 1)
        target_layer_ids = self._training_value(training_cfg, "domino_target_layer_ids", "dflash_target_layer_ids", None)
        ...
        return DominoConfig(
            ...
            num_hidden_layers=int(self._training_value(training_cfg, "domino_num_hidden_layers", "dflash_num_hidden_layers", 5)),
            ...
            pure_draft_prefix_len=int(training_cfg.get("domino_pure_draft_prefix_len", 1)),
            ...
            lambda_base_decay_steps=int(training_cfg.get("domino_lambda_base_decay_steps", 2000)),
    ...
    def build_model(self):
        ...
        if config_path and os.path.exists(config_path):
            ...
            if spec_model_path and os.path.exists(spec_model_path):
                log_drafter_checkpoint_step(logger, spec_model_path, action="Loading Domino drafter weights")
        ...
        if not isinstance(drafter_config, DominoConfig):
            raise TypeError(f"Domino config is not a DominoConfig: {type(drafter_config)}")
