...
def main() -> None:
    ...
    print(
        f"馃攳 Type coverage on {'all' if args.all_lines else 'changed'} lines: "
        f"{total_annotated}/{total_changed} = {ratio:.2%}. Files inspected: {target_files}"
    )

    if all_warnings:
        print(
            "\n鈿狅笍 Suggest Improve: Lines missing type annotations for inputs and outputs:\n"
        )
        for fname, lineno, line in all_warnings:
            print(f"{fname}:{lineno}: {line}")

    if all_failures:
        print("鈿狅笍 [ERROR] Lines missing type annotations for inputs and outputs:\n")
    ...
    if ratio < args.threshold:
        ...
        raise Exception(
            f"\n[FAIL] Type coverage below threshold ({args.threshold:.0%})."
        )
    else:
        if all_warnings or all_failures:
            print("")
        print("[OK] Type annotation coverage acceptable.\n")
