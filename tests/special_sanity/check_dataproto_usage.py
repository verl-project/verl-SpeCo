...
"""Check that DataProto usage stays inside SpeCo's upstream-verl adapter boundary."""
    ...
    for path in pathlist:
        ...
        for sw in SEARCH_WHITELIST:
            ...
            if sw in path_in_str:
                print(
                    f"[SKIP] File {path_in_str} is in DataProto usage check "
        ...
        with open(path_in_str, encoding="utf-8") as f:
            ...
            assert not find_invalid_device_management, (
                f"file {path_in_str} contains DataProto usage outside the "
                "SpeCo verl compatibility boundary. Use TensorDict or add a "
                "documented adapter-boundary whitelist entry."
            )
