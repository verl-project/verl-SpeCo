...
if not re_modules:
    print(f"[FAIL] Invalid PR title: '{pr_title}'")
    ...
    if not all(module in allowed_modules for module in modules):
        ...
        print(f"[FAIL] Invalid modules: {', '.join(invalid_modules)}")
        print(f"Allowed modules: {', '.join(allowed_modules)}")
        raise Exception("Invalid PR title")
...
print(
    f"[OK] PR title is valid: {pr_title}, modules: {modules}, type: {change_type}{breaking_info}"
)
