...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--target-file",
        default="verl_speco/trainer/base_trainer.py",
        help="Path to the Python source file to analyse (e.g. verl_speco/trainer/base_trainer.py)",
    )
    p.add_argument(
        "--allow-list",
...
def main() -> None:
    ...
    if not target_path.is_file():
        raise Exception(f"[FAIL] Target file not found: {target_path}")
    ...
    if errors:
        ...
        raise Exception("[FAIL] Docstring verification failed.")

    if not args.quiet:
        print(
            f"[OK] All explicitly imported functions/classes in {target_path} have docstrings."
        )
