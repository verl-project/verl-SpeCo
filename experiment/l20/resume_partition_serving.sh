...
out=$run/evidence/full-e2e/partition-serving-resume
...
for arm in flat partition; do
  ...
  "$py" -m verl_speco.convert_peagle_vllm "$checkpoint" "$fixture/converted-$arm-resume" \
  ...
  timeout -k 15 900 "$py" "$source/experiment/l20/check_tiny_peagle_serving.py" draft-eagle3 \
    ...
    --draft-model "$fixture/converted-$arm-resume" --reference-model "$checkpoint" \
