...
out=$run/evidence/full-e2e/head64-serving-v2
mkdir -p "$out"
trap 'echo $? > "$out/suite.exit"' EXIT
"$py" -m verl_speco.convert_peagle_vllm "$root/evidence/l20-20260919/native-head64-veomni/draft_step_6" "$run/full-e2e-head64/converted-veomni-v2" --target-layer-ids 0 1 2 > "$out/convert.log" 2>&1
for tp in 1 2; do
  backend=()
  ...
  for mode in baseline draft-eagle3; do
    ...
    timeout -k 15 900 "$py" "$source/experiment/l20/check_tiny_peagle_serving.py" "$mode" \
      --tp "$tp" "${backend[@]}" --models-root "$run/full-e2e-head64" --draft-model "$run/full-e2e-head64/converted-veomni-v2" \
      --reference-model "$root/evidence/l20-20260919/native-head64-veomni/draft_step_6" --evidence-root "$out" \
  ...
  if [[ "$tp" == 1 ]]; then
    timeout -k 15 900 "$py" "$source/experiment/l20/check_peagle_decode_logits.py" \
      --prefix native-tp1 --reference "$root/evidence/l20-20260919/native-head64-veomni/draft_step_6" --evidence-root "$out" > "$out/decode-parity.log" 2>&1
  fi
done
for tp in 1 2; do
  backend=()
  ...
  for mode in baseline draft-eagle3; do
    timeout -k 15 900 "$py" "$source/experiment/l20/check_tiny_peagle_serving.py" "$mode" \
      --tp "$tp" "${backend[@]}" --graph --no-capture --models-root "$run/full-e2e-head64" \
      --draft-model "$run/full-e2e-head64/converted-veomni-v2" --reference-model "$root/evidence/l20-20260919/native-head64-veomni/draft_step_6" \
