...
PYTHONUNBUFFERED=1 VERL_DRAFTER_ALIGNMENT_DEBUG=1 VERL_DRAFTER_ALIGNMENT_DEBUG_EVERY_N_STEPS=1 VERL_DRAFTER_LAST_HIDDEN_LOGPROB_CHECK=1 RAY_TMPDIR=/home/model/tmp/ray_tmpdir python3 -m verl_speco.main \
    ...
    trainer.test_freq=5 \
    trainer.total_epochs=1 $@ 2>&1 | tee /verl-SpeCo/logs/${exp_name}.log &

TRAIN_PID=$!
echo "Training PID: $TRAIN_PID, log: /verl-SpeCo/logs/${exp_name}.log"
echo "Collect interval: 5 steps, Training interval: 5 steps, Total epochs: 1"
echo "Training will run for a full epoch. Press Ctrl+C or kill $TRAIN_PID to stop."
wait $TRAIN_PID
